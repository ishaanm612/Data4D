...
Addition_Vechicles = {  # only available in latest UE5.5 package
    "car": [
        "BP_Hatchback_child_extras_C",
        "BP_Hatchback_child_police_C",
        "BP_Hatchback_child_taxi_C",
        "BP_Sedan_child_base_C",
        "BP_Sedan_child_extras_C",
        "BP_Sedan_child_police_C",
        "BP_Sedan_child_taxi_C",
        "BP_SUV_child_base_C",
        "BP_SUV_child_extras_C",
        "BP_SUV_child_police_C",
        "BP_SUV_child_taxi_C",
    ],
    "motorbike": [
        "BP_Custom_Base_C",
...
animal_config = {
    ...
    "move_action_continuous": {"high": [30, 200], "low": [-30, -200]},
}
...
car_config = {
    ...
    "relative_location": [0, 0, 0],
    "relative_rotation": [0, 0, 0],
    "move_action": [[1, 0], [-0.3, 0], [0.5, 1], [0.5, -1], [0, 0]],
    "move_action_continuous": {"high": [1, 1], "low": [0, -1]},
}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--env-bin",
        default="UnrealZoo_UE5_5_Linux/Linux/UnrealZoo_UE5_5/Binaries/Linux/UnrealZoo_UE5_5",
        help="The path to the UE4Editor binary",
    )
    ...
    parser.add_argument(
        "--target_dir",
        default="gym_unrealcv/envs/setting/Track",
        help="The folder to save the json file",
    )

    parser.add_argument(
        "--use-docker", action="store_true", help="Run the game in a docker container"
    )
    parser.add_argument(
        "--resolution",
        "-res",
        default="640x480",
        help="The resolution in the unrealcv.ini file",
    )
    parser.add_argument("--display", default=None, help="The display to use")
    parser.add_argument(
        "--use-opengl", action="store_true", help="Use OpenGL for rendering"
    )
    parser.add_argument(
        "--offscreen", action="store_true", help="Use offscreen rendering"
    )
    ...
    if args.env_map == "all":
        maps = [
            ...
            "RuralAustralia_Example_02",
            "RuralAustralia_Example_03",
            "LV_Soul_Cave",
            "Dungeon_Demo_00",
            "SwimmingPool",
            "DesertMap",
            ...
            "AsianMedivalCity",
            "StonePineForest",
            "TemplesOfCambodia_01_01_Exterior",
            "AbandonedDistrict",
        ]
        env_map = maps[0]
    else:
    ...
    unrealcv = UnrealCv_API(
        env_port, env_ip, parse_resolution(args.resolution), "tcp"
    )  # 'tcp' or 'unix', 'unix' is only for local machine in Linux
    ...
    for env_map in maps:
        ...
        env_config["env_bin_win"] = ue_binary.env_bin.replace("/", "\\").replace(
            "Linux", "Win"
        )
        ...
        for obj in objects:
            if re.match(re.compile(r"bp_character", re.I), obj) is not None:
                ...
                re.match(re.compile(r"bp_basecar|BP_Hatchback", re.I), obj) is not None
            ):
                ...
                re.match(re.compile(r"sport|motorbike|BP_BaseBike", re.I), obj)
                is not None
            ):
                ...
                re.match(re.compile(r"bp_door", re.I), obj) is not None
                or re.match(re.compile(r"animateddoor", re.I), obj) is not None
            ):
                env_config["env"]["interactive_door"].append(obj)

        agents = {
            k: v for k, v in agents.items() if len(v["name"]) > 0
        }  # remove the agent category not in the scene
