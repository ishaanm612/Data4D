...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--env_id",
        nargs="?",
        default="UnrealNavigationMulti-SuburbNeighborhood_Day-ContinuousColor-v0",
        help="Select the environment to run",
    )
    parser.add_argument(
    ...
    env.unwrapped.agents_category = [
        "player",
        "drone",
    ]  # choose the agent type in the scene
    ...
    ]  # use random policy as the base policy for each agent
    print(
        """Use the "I", "J", "K", and "L" keys to control the first agent's movement, "Space" to jump, and "Up" and "Down" to adjust the agents view. (Double "Jump" will trigger the agent to climb)"""
    )
    while True:

        actions = [
            random_agents[i].act(obs[i]) for i in range(agents_num)
        ]  # assign random policy for all agents
        actions[0] = (
            get_key_action_continuous()
        )  # overwrite keyboard control policy for the first agent's movement (human character)
