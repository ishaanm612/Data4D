...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--env_id",
        nargs="?",
        default="UnrealTrack-FlexibleRoom-ContinuousColor-v0",
    ...
    env = agents.NavAgents(
        env, mask_agent=True
    )  # use internal nav system for target moving
    episode_count = 100
    rewards = 0
    done = False
    ...
    try:
        for eps in range(1, episode_count):
            ...
            print(
                f"Tracker ID: {tracker_id} ({env.unwrapped.agents_category[tracker_id]}), Target ID: {target_id} ({env.unwrapped.agents_category[target_id]})"
            )
