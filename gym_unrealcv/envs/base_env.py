...
class UnrealCv_base(gym.Env):
    ...
    def __init__(
        ...
        self.start_area = self.get_start_area(
            self.safe_start[0], 500
        )  # the start area of the agent, where we don't put obstacles
        ...
        self.action_space = [
            self.define_action_space(self.action_type, self.agents[obj])
        ...
        assert self.observation_type in [
            "Color",
            "Depth",
            "Rgbd",
            "Gray",
        ...
        self.observation_space = [
            self.define_observation_space(
                self.cam_list[i], self.observation_type, resolution
            )
    ...
    def step(self, actions):
        ...
        actions2move, actions2turn, actions2animate = self.action_mapping(
            actions, self.player_list
        )
        move_cmds = [
        ...
        head_cmds = [
            self.unrealcv.set_cam(
                obj,
                self.agents[obj]["relative_location"],
                actions2turn[i],
                return_cmd=True,
            )
            for i, obj in enumerate(self.player_list)
        ...
        observations = self.prepare_observation(
            self.observation_type, imgs, masks, depths, obj_poses
        )
    ...
    def reset(self):
        ...
        for i, obj in enumerate(self.player_list):
            ...
            self.unrealcv.set_cam(
                obj,
                self.agents[obj]["relative_location"],
                self.agents[obj]["relative_rotation"],
            )
    ...
    def update_observation(self, player_list, cam_list, cam_flag, observation_type):
        ...
        """
        obj_poses, cam_poses, imgs, masks, depths = self.unrealcv.get_pose_img_batch(
            player_list, cam_list, cam_flag
        )
        observations = self.prepare_observation(
            observation_type, imgs, masks, depths, obj_poses
        )
    ...
    def get_start_area(self, safe_start, safe_range):
        ...
        """
        start_area = [
            safe_start[0] - safe_range,
            safe_start[0] + safe_range,
            safe_start[1] - safe_range,
            safe_start[1] + safe_range,
        ]
        return start_area
    ...
    def get_relative(self, pose0, pose1):  # pose0-centric
        ...
        obs_vector = [
            np.sin(delt_yaw / 180 * np.pi),
            np.cos(delt_yaw / 180 * np.pi),
    ...
    def prepare_observation(
        self, observation_type, img_list, mask_list, depth_list, pose_list
    ):
        """
    ...
    def rotate2exp(self, yaw_exp, obj, th=1):
        ...
        while abs(delta_yaw) > th:
            if "Drone" in obj:
                self.unrealcv.set_move_bp(
                    obj, [0, 0, 0, np.clip(delta_yaw, -60, 60) / 60 * np.pi]
                )
            else:
    ...
    def relative_metrics(self, relative_pose):
        ...
        collision_mat[np.where(np.fabs(relative_ori) > 45)] = (
            0  # collision should be at the front view
        )
        info["collision"] = collision_mat
        info["dis_ave"] = (
            relative_dis.mean()
        )  # average distance among players, regard as a kind of density metric
    ...
    def add_agent(self, name, loc, refer_agent):
        ...
        self.unrealcv.new_obj(
            refer_agent["class_name"], name, random.sample(self.safe_start, 1)[0]
        )
        ...
        self.action_space.append(
            self.define_action_space(self.action_type, agent_info=new_dict)
        )
        self.observation_space.append(
            self.define_observation_space(
                new_dict["cam_id"], self.observation_type, self.resolution
            )
        )
    ...
    def define_action_space(self, action_type, agent_info):
        ...
        if action_type == "Discrete":
            ...
            return spaces.Box(
                low=np.array(agent_info["move_action_continuous"]["low"]),
                high=np.array(agent_info["move_action_continuous"]["high"]),
                dtype=np.float32,
            )
        else:  # Hybrid
            move_space = spaces.Box(
                low=np.array(agent_info["move_action_continuous"]["low"]),
                high=np.array(agent_info["move_action_continuous"]["high"]),
                dtype=np.float32,
            )
    ...
    def define_observation_space(self, cam_id, observation_type, resolution=(160, 120)):
        ...
        if observation_type == "Pose" or cam_id < 0:
            observation_space = spaces.Box(
                low=-100, high=100, shape=(6,), dtype=np.float16
            )  # TODO check the range and shape
        else:
            if (
                observation_type == "Color"
                or observation_type == "CG"
                ...
                observation_space = spaces.Box(
                    low=0, high=100, shape=img_shape, dtype=np.float16
                )
            elif observation_type == "Rgbd":
    ...
    def sample_init_pose(self, use_reset_area=False, num_agents=1):
        ...
        if num_agents > len(self.safe_start):
            use_reset_area = True
            warnings.warn(
                "The number of agents is less than the number of pre-defined start points, random sample points from the pre-defined area instead."
            )
        if use_reset_area:
            locations = self.sample_from_area(
                self.reset_area, num_agents
            )  # sample from a pre-defined area
        else:
            locations = random.sample(
                self.safe_start, num_agents
            )  # sample one pre-defined start point
        return locations
    ...
    def random_app(self):
        ...
        """
        app_map = {"player": range(1, 19), "animal": range(0, 27), "drone": range(0, 1)}
        for obj in self.player_list:
    ...
    def environment_augmentation(
        self,
        player_mesh=False,
        player_texture=False,
        light=False,
        background_texture=False,
        layout=False,
        layout_texture=False,
    ):
        """
        Randomly assign an appearance to each agent in the player list based on their category.
        ...
        if background_texture:
            self.unrealcv.random_texture(
                self.env_configs["backgrounds"], self.textures_list, 5
            )

        # random place the obstacle`
        if layout:
            self.unrealcv.clean_obstacles()
            self.unrealcv.random_obstacles(
                self.objects_list,
                self.textures_list,
                random.randint(5, len(self.objects_list)),
                self.reset_area,
                self.start_area,
                layout_texture,
            )
    ...
    def init_agents(self):
        for (
            obj
        ) in self.player_list.copy():  # the agent will be fully removed in self.agents
    ...
    def set_population(self, num_agents):
        while len(self.player_list) < num_agents:
            ...
            self.agents[name] = self.add_agent(
    ...
    def set_npc(self):
        ...
        return self.player_list.index(
            random.choice([x for x in self.player_list if x > 0])
        )
            ...
            if isinstance(action_space, spaces.Discrete):
                ...
                for j, action in enumerate(actions[i]):
                    if j == 0:
                        ...
                        actions2animate.append(
                            self.agents[obj]["animation_action"][action]
                        )
    ...
    def get_cam_flag(
        ...
        flag[1] = (
            observation_type == "Color"
            or observation_type == "Rgbd"
            or use_color
            or observation_type == "ColorMask"
        )
        flag[2] = (
