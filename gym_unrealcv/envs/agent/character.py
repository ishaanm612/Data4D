...
class Character_API(UnrealCv_API):
    def __init__(
        self, port=9000, ip="127.0.0.1", resolution=(160, 120), comm_mode="tcp"
    ):
        super(Character_API, self).__init__(
            port=port, ip=ip, resolution=resolution, mode=comm_mode
        )
        self.obstacles = []
        self.targets = []
    ...
    def random_obstacles(self, objects, img_dirs, num, area, start_area, texture=False):
        ...
        for id in sample_index:
            ...
            while (
                start_area[0] <= obstacle_loc[0] <= start_area[1]
                and start_area[2] <= obstacle_loc[1] <= start_area[3]
            ):
    ...
    def new_obj(self, obj_class_name, obj_name, loc, rot=[0, 0, 0]):
        ...
        if obj_class_name == "bp_character_C" or obj_class_name == "target_C":
            cmd = [
                f"vset /objects/spawn {obj_class_name} {obj_name}",
    ...
    def adjust_fov(self, cam_id, delta_fov, min_max=[45, 135]):  # increase/decrease fov
        return self.set_fov(
            cam_id, np.clip(self.cam[cam_id]["fov"] + delta_fov, min_max[0], min_max[1])
        )
    ...
    def nav_to_obj(
        self, obj, target_obj, distance=200
    ):  # navigate the agent to a target object
        # Assign the agent a navigation goal, and use Navmesh to automatically control its movement to reach the goal via the shortest path.
    ...
    def generate_nav_goal(
        self, player, radius_max, radius_min
    ):  # navigate the agent to a random location
        # Agent randomly selects a point within its own radius range for navigation.
        # The loop parameter controls whether continuous navigation is performed.（True for continuous navigation).
        # Return with the randomly sampled location.
    ...
    def set_max_nav_speed(
        self, obj, max_vel
    ):  # set the maximum navigation speed of the car
    ...
    def get_pose_img_batch(
        self, objs_list, cam_ids, img_flag=[False, True, False, False]
    ):
        ...
        for obj in objs_list:
            cmd_list.extend(
                [self.get_obj_location(obj, True), self.get_obj_rotation(obj, True)]
            )
        ...
        for cam_id in cam_ids:
            ...
            if use_mask:
                cmd_list.append(
                    self.get_image(cam_id, "object_mask", "png", return_cmd=True)
                )
            if use_depth:
        ...
        decoders = [
            self.decoder.decode_map[self.decoder.cmd2key(cmd)] for cmd in cmd_list
        ]
        res_list = []
        try:
            res_list = self.batch_cmd(cmd_list, decoders)
        except Exception as e:
            print(f"batch cmd error: {e}")
            return [], [], [], [], []
        ...
        for i, cam_id in enumerate(cam_ids):
            ...
            if use_depth:
                ...
                depth_list.append(
                    image
                )  # 500 is the default max depth of most depth cameras
    ...
    def set_texture(
        self, player, color=(1, 1, 1), param=(0, 0, 0), picpath=None, tiling=1, e_num=0
    ):  # [r, g, b, meta, spec, rough, tiling, picpath]
